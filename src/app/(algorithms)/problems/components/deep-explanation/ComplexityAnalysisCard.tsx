    ...
    <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-xl p-6 border border-zinc-200 dark:border-zinc-700/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-gradient-to-br from-green-900/30 to-emerald-900/30 rounded-lg p-4 border border-green-500/30">
          ...
          <p className="text-sm text-zinc-700 dark:text-zinc-300">{analysis.timeBreakdown}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-900/30 to-indigo-900/30 rounded-lg p-4 border border-blue-500/30">
          ...
          <p className="text-sm text-zinc-700 dark:text-zinc-300">{analysis.spaceBreakdown}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          {analysis.bestCase && (
            <div className="bg-black/20 rounded-lg p-3">
              <span className="text-sm text-zinc-600 dark:text-zinc-400">🌟 最优情况</span>
              <p className="text-zinc-700 dark:text-zinc-300 mt-1">{analysis.bestCase}</p>
            </div>
          )}
          {analysis.worstCase && (
            <div className="bg-black/20 rounded-lg p-3">
              <span className="text-sm text-zinc-600 dark:text-zinc-400">⚡ 最差情况</span>
              <p className="text-zinc-700 dark:text-zinc-300 mt-1">{analysis.worstCase}</p>
            </div>
          )}
        </div>
      )}
      ...
      {analysis.reasoning && (
        <div className="bg-purple-500/10 rounded-lg p-4 border border-purple-500/30">
          ...
          <p className="text-zinc-700 dark:text-zinc-300 mt-2">{analysis.reasoning}</p>
        </div>
      )}

      {/* 复杂度对比图示 */}
      <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-700/50">
        <span className="text-sm text-zinc-600 dark:text-zinc-400 mb-2 block">复杂度等级参考</span>
