...
export function solution(nums, k) {
  ...
  let newK1 = k % nums.length;
  console.log('有效轮转', newK);
  const resArr = [];
  while (newK) {
    resArr.push(nums[nums.length - newK]);
    newK--;
  }
  console.log("resArr", resArr, nums.length - newK1);
  // console.log("剩余补充的数组", nums.splice(0, nums.length - newK1));
  resArr.push(...nums.splice(0, nums.length - newK1));
