...
export function solution(nums1, m, nums2, n) {
  ...
  // 主循环: 从后往前比较两个数组的元素，较大的放到 resLength 位置
  // 条件: 两个数组都还有未处理的元素
  while (mIndex >= 0 && nIndex >= 0) {
    if (nums1[mIndex] > nums2[nIndex]) {
      ...
      // nums2 当前元素更大或相等，放到末尾
      // 相等时优先放 nums2 的元素 (不影响结果正确性)
  ...
  // 收尾: 如果 nums2 还有剩余元素，逐个拷贝到 nums1 前面
  // 注意: 如果 nums1 还有剩余 (mIndex >= 0)，不需要处理，因为它们已经在正确位置上
  while (nIndex >= 0) {
    nums1[resLength] = nums2[nIndex];
    nIndex--;
    resLength--;
